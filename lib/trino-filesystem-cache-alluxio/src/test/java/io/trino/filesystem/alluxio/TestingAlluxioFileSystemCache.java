        ...
        extends AlluxioFileSystemCache
{
    ...
    {
        super(AlluxioFileSystemCacheModule.getCacheManager(alluxioConfiguration), alluxioConfiguration, new TestingAlluxioCacheStats());
        statistics = (TestingAlluxioCacheStats) getStatistics();
        this.delegateKeyProvider = requireNonNull(keyProvider, "keyProvider is null");
            ...
            extends AlluxioCacheStats
    {
        ...
        {
            super.recordExternalRead(location, length);
            operationCounts.merge(new OperationContext(OperationType.EXTERNAL_READ, location), 1, Math::addExact);
        }

        @Override
        public void recordCacheRead(Location location, int length)
        {
            super.recordCacheRead(location, length);
            operationCounts.merge(new OperationContext(OperationType.CACHE_READ, location), 1, Math::addExact);
        }
