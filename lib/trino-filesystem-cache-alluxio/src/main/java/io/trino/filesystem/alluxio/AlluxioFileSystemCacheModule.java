...
import static alluxio.conf.PropertyKey.USER_CLIENT_CACHE_SHADOW_ENABLED;
        ...
        extends AbstractConfigurationAwareModule
{
    ...
    protected void setup(Binder binder)
    {
        ...
        newOptionalBinder(binder, CachingHostAddressProvider.class).setBinding().to(ConsistentHashingCachingHostAddressProvider.class).in(SINGLETON);
    ...
    public static AlluxioConfiguration getAlluxioConfiguration(AlluxioFileSystemCacheConfig config)
    {
        checkArgument(config.getMaxCacheSizes().isEmpty() ^ config.getMaxCacheDiskUsagePercentages().isEmpty(),
                format("Either %s or %s must be specified", CACHE_MAX_SIZES, CACHE_MAX_PERCENTAGES));
        ...
        checkArgument(config.getCacheDirectories().size() == size,
                format("%s and %s must have the same size", CACHE_DIRECTORIES, config.getMaxCacheSizes().isEmpty() ? CACHE_MAX_PERCENTAGES : CACHE_MAX_SIZES));
        ...
        alluxioProperties.set(USER_CLIENT_CACHE_SHADOW_ENABLED, config.isShadowCacheEnabled());
        alluxioProperties.set(USER_CLIENT_CACHE_TTL_THRESHOLD_SECONDS, config.getCacheTTL().roundTo(TimeUnit.SECONDS));
    ...
    private static List<DataSize> calculateMaxCacheSizes(List<Double> cachePercentages, List<Long> cacheDiskSizes)
    {
        ...
        for (int i = 0; i < cacheDiskSizes.size(); i++) {
            maxCacheSizes.add(DataSize.of(Math.round(cachePercentages.get(i) * cacheDiskSizes.get(i)), DataSize.Unit.BYTE));
        }
