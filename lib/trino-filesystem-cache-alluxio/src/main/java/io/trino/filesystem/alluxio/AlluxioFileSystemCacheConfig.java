...
public class AlluxioFileSystemCacheConfig
{
    ...
    @ConfigDescription("The maximum percentage (0-100) of total disk size the cache can use. A comma-separated list of percentages if supplying several cache directories")
    public AlluxioFileSystemCacheConfig setMaxCacheDiskUsagePercentages(String maxCacheDiskUsagePercentages)
    {
        ...
        values.forEach(p -> checkArgument(0 <= p && p <= 1, format("Percentage %f must be between 0 and 100", p * 100)));
