...
public class AlluxioInputHelper
{
    ...
    public AlluxioInputHelper(Location location, URIStatus status, CacheManager cacheManager, AlluxioConfiguration configuration, AlluxioCacheStats statistics)
    {
        ...
        // Buffer to reduce the cost of doing page aligned reads
        this.bufferSize = 8 * 1024;
        this.readBuffer = new byte[bufferSize];
    }

    public int doBufferRead(long position, byte[] bytes, int offset, int length)
    {
        if (length == 0) {
            return 0;
        }
        ...
        int bytesToCopy = min(length, Ints.saturatedCast(Long.max(bufferEnd - position, 0)));
        System.arraycopy(readBuffer, Ints.saturatedCast(position - bufferStart), bytes, offset, bytesToCopy);
        return bytesToCopy;
    }
            ...
            throws IOException
    {
        ...
        if (length - remainingLength > 0) { // To keep tests fast
            statistics.recordCacheRead(location, length - remainingLength);
        }
    ...
    private int putBuffer(long position, int pageOffset, PageId pageId, CacheContext cacheContext)
    {
        ...
        bufferStart = pageId.getPageIndex() * pageSize + pageOffset;
    ...
    public void putCache(long pageStart, long pageEnd, byte[] readBuffer, int length)
    {
        ...
        while (pageStart < end) {
            ...
            CacheContext cacheContext = status.getCacheContext();
            PageId pageId = new PageId(cacheContext.getCacheIdentifier(), currentPage);
            cacheManager.put(pageId, ByteBuffer.wrap(readBuffer, 0, currentPageSize));
